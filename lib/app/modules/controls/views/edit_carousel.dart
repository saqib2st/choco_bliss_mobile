...
class EditCarousel extends GetView<ControlsController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Carousel Images",style: TextStyle(
          fontSize: 15
        ),),
        actions: [
          Container(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: Obx(() {
              if (controller.imageListCarousel.isEmpty) {
                return const Center(child: Text("No images selected"));
              }
              return GridView.builder(
                itemCount: controller.imageListCarousel.length,
                ...
                itemBuilder: (context, index) {
                  return Stack(
                    children: [
                      Image.file(
                        controller.imageListCarousel[index],
                        fit: BoxFit.cover,
          ...
          Padding(
            ...
            child: ElevatedButton.icon(
                ...
                onPressed: ()
                {
                  controller.pickImageFromGallery(controller.imageListCarousel);
                } // Upload action
            ),
          ),
                        ...
                        child: GestureDetector(
                          onTap: () {
                            controller.deleteImage(index,controller.fetchedList);
                                controller.deleteSingleImages(index,'slider');
                          },
                          child: const Icon(Icons.delete, color: Colors.red),
                        ),
                      ),
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: ()
              {
                if (controller.fetchedList.isNotEmpty) {
                  controller.deleteOldImages('slider');
                }else{
